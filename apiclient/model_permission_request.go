...
type PermissionRequest struct {
	// The role name, which cannot exceed 255 characters.
	ApiName string `json:"apiName"`
	// The allowed action for the API endpoint, with CREATE typically corresponding to POST endpoints, READ to GET endpoints, UPDATE to PATCH/PUT endpoints, and DELETE to DELETE endpoints.
	Actions []string `json:"actions"`
	// These are the tag IDs that determine resource access. Access to a resource will only be granted to users with the specified role if the associated tags are assigned to that resource.
