...
Create a new secret with the attributes 'name', 'type' (password or ssh) and 'value'.
...
/*
DeleteSecret Delete existing secret by its identifier

Selected secrets cam be removed from your account.
...
Get the attribute values associated with a particular secret in your account.
...
// Secrets (can be filtered by their name).
...
/*
RetrieveSecretList Show secrets

Displays all secrets of your account using filters.
...
/*
UpdateSecret Update specific secret by its identifier

Modify the attributes associated with your secret, but keep in mind that attributes are not mandatory. If no attributes are specified, the existing attribute values will remain unchanged. Note that only the name and value of the attributes can be updated.
